...
namespace TinyUtilities.Vibration {
    ...
    #if UNITY_ANDROID && !UNITY_EDITOR || VIBRATION_DEBUG
        ...
        public static void CreateOneShot(long milliseconds, int amplitude) {
                ...
                if (_isSupportVibrationEffect) {
                    if (_hasAmplitudeControl) {
                        using AndroidJavaObject createOneShot =
 _vibrationEffect.CallStatic<AndroidJavaObject>(_CREATE_ONE_SHOT_METHOD, milliseconds, amplitude);
                        _vibrator.Call(_VIBRATE_METHOD, createOneShot);
                    } else {
                        using AndroidJavaObject createOneShot = _vibrationEffect.CallStatic<AndroidJavaObject>(
                            _CREATE_ONE_SHOT_METHOD, milliseconds, _defaultAmplitude
                        );
                        _vibrator.Call(_VIBRATE_METHOD, createOneShot);
                    }
                } else
                    _vibrator.Call(_VIBRATE_METHOD, milliseconds);
        #elif UNITY_IOS && !UNITY_EDITOR || VIBRATION_DEBUG
            Handheld.Vibrate();
        #else
            Handheld.Vibrate();
        #endif
        ...
        public static void CreatePredefined(PredefinedEffect effect) {
        #if UNITY_ANDROID && !UNITY_EDITOR || VIBRATION_DEBUG
                switch (effect) {
                    case PredefinedEffect.DoubleClick:
                        CreateWaveform(new long[] { 0, 64, 0, 64 });

                        break;

                    case PredefinedEffect.Tick:
                        CreateOneShot(32);

                        break;

                    case PredefinedEffect.HeavyClick:
                        CreateOneShot(128);
            ...
            public static void CreateWaveform(long[] timings) {
                ...
                for (int amplitudeId = 2; amplitudeId < amplitudes.Length; amplitudeId += 2) amplitudes[amplitudeId] = _defaultAmplitude;
        ...
        public static void CreateWaveform(long[] timings, int amplitude) {
            int[] amplitudes = new int[timings.Length];
            for (int amplitudeId = 2; amplitudeId < amplitudes.Length; amplitudeId += 2) amplitudes[amplitudeId] = amplitude;
            CreateWaveform(timings, amplitudes, -1);
        }
        ...
        public static void CreateWaveform(long[] timings, int[] amplitudes, int repeat) {
                ...
                for (int timingId = 1; timingId < timings.Length; timingId += 2) {
                    if (timings[timingId] < 1) timings[timingId] = 1;

                    if (amplitudes[timingId] < 1)
                        amplitudes[timingId] = 1;
                    else if (amplitudes[timingId] > 255) amplitudes[timingId] = 255;
                }

                if (_isSupportVibrationEffect) {
                    if (_hasAmplitudeControl) {
                        using AndroidJavaObject createWaveform = _vibrationEffect.CallStatic<AndroidJavaObject>(
                            _CREATE_WAVEFORM_METHOD, timings, amplitudes, repeat
                        );
